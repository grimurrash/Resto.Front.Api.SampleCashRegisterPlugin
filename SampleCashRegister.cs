...
{
    internal sealed class SampleCashRegister : MarshalByRefObject, ICashRegister
    {
        ...
        private const string DeviceFriendlyName = "FP700";
        private State state = State.Stopped;
        private FRgeorgia FR;
        ...
        public void Stop()
        {
            PluginContext.Log.InfoFormat("Device: '{0} ({1})' was stopped (84)", DeviceName, deviceId);
            try
            {
                FR.Dispose();
            }
            ...
            {
                PluginContext.Log.WarnFormat("Device: '{0} ({1})' Error: {2} (91)", DeviceName, deviceId, ex.Message);
            }
            state = State.Stopped;
        }
        ...
        public void Setup([NotNull] DeviceSettings newSettings)
        {
            ...
            PluginContext.Log.InfoFormat("Device: '{0} ({1})' was setup (104)", DeviceName, deviceId);
        }

        public void RemoveDevice()
        {
            FR.Dispose();
            PluginContext.Log.InfoFormat("Device: '{0} ({1})' was removed (110)", DeviceName, deviceId);
        }
        ...
        {
            PluginContext.Log.InfoFormat("Device: '{0} ({1})'. Get query info. (367)", DeviceName, deviceId);
        ...
        public CashRegisterResult DoBillCheque(BillTask billTask, IViewManager viewManager)
        {
            ...
            PluginContext.Log.InfoFormat("Device: '{0} ({1})'. BillCheque. (403)", DeviceName, deviceId);
            return GetCashRegisterData();
        }
        ...
        {
            PluginContext.Log.InfoFormat("Device: '{0} ({1})'. CustomerDisplayIdle. (418)", DeviceName, deviceId);
        ...
        { 
            PluginContext.Log.InfoFormat("Device: '{0} ({1})'. CustomerDisplayText. (430)", DeviceName, deviceId);
        ...
        /// <see cref="CashRegisterDriverParameters.SupportsPayInAfterSessionClose"/>Поддерживает ли ККМ внесение при закрытой кассовой смене<para />
        ...
        public CashRegisterDriverParameters GetCashRegisterDriverParameters()
        {
            ...
            {
                CanPrintText = true,
                SupportsPayInAfterSessionClose = true,
